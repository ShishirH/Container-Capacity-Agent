...
class Analysis {
  static void analyzeMemory(JSONObject memoryAnalysisObject, MetricCollector metricCollector) {
    ...
    MetricCollector.chartResidentStat.addValue(
        Util.convertToMB(metricCollector.getResidentMemoryStat().getPercentile(50)));
  ...
  static void analyzeCPU(JSONObject cpuAnalysisObject, MetricCollector metricCollector) {
    ...
    MetricCollector.chartCpuLoadStat.addValue(
        metricCollector.cpuMetricsImpl.getCpuLoad().getPercentile(50));
    CpuMetricsImpl.cpuSecondsStat.addValue(extendedOperatingSystemMXBean.getProcessCpuTime());
  }

  private static void addStats(
      boolean isMemory, Map<String, String> map, DescriptiveStatistics statistics) {
    ...
    map.put("Median", Double.toString((statistics.getPercentile(50) / div)));
    map.put("Mean", Double.toString(mean / div));
    map.put("StandardDeviation", Double.toString((statistics.getStandardDeviation() / div)));
